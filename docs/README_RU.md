...
![Сравнение Polyglot Keeper](./screenshots/comparison.png)
...
- **Визуальный редактор** — встроенный веб-интерфейс (`npx polyglot-keeper serve`): перевод ключей с помощью ИИ, добавление/удаление локалей, поиск и сохранение прямо в браузере.
- **Ротация API-ключей** — поддержка пула ключей для автоматического обхода лимитов rate limit (`429`) в CLI, UI и Runtime.
...
### 3. Добавьте API-ключ(и)

```bash
# .env — поддерживает несколько ключей через запятую для авто-ротации и обхода лимитов (429)
POLYGLOT_API_KEY=ключ_1,ключ_2,ключ_3
```
...
![Визуальный редактор Polyglot Keeper](./screenshots/ui.png)

Запустите локальный веб-интерфейс для управления файлами локалей — без внешних сервисов и подписок:

```bash
npx polyglot-keeper serve
```

Команда автоматически откроет `http://localhost:3636` в браузере. Возможности:

- **Карточки переводов** — удобное редактирование с предпросмотром источника и отслеживанием черновиков.
- **Фильтрация и поиск** — быстрый поиск и фильтры: `All`, `Missing` (только непереведённые), `Drafts` (несохранённые изменения).
- **ИИ-перевод** — перевод отдельного ключа (✦) или пакетный перевод всех недостающих ключей.
- **Управление языками и ключами** — добавление новых локалей и ключей, удаление с подтверждением, смена основного языка (`Set as Source`).
